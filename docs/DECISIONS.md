...
| L-11 | **HSK1 词表尚未录入**，`pipeline/wordlists/zh/hsk1.txt` 不存在，规则 1「词表越界」因此跑不了真实草稿——`make validate` 退出码 2。这是设计如此（D-089：缺数据必须硬失败，不能静默通过），但它意味着 **M1 验收前必须补上这份数据**，否则整条管线里唯一守住「不超纲」的规则是空的。GF 0025-2021 的一级词表带 1–500 连续编号且附拼音，抽取可以自校验（编号不断、止于 500、拼音与 `pypinyin` 交叉比对）。 | E6 打包前；最迟 M1 验收 | `pipeline/wordlists/README.md`；`pipeline/wordlist.py` 的 `MissingWordlistError` |

...
## D-051 i18n 的结构检查是 PR 闸门，占位符数量只报数

- 日期：2026-09-19
- 背景：BACKLOG D7 的验收写的是「CI 检查 km 无占位符」。
  照字面实现是每个 PR 硬失败——而现在 25 个 key 全是占位符，
  翻译要等母语者（PRD 15.3 第 3 条），中间的 D8/D9/D10 全部合不了。
  由项目所有者于 2026-09-19 拍板拆成两层。
- 选择：
  - **每个 PR 硬卡三条结构性检查**（在 `make check` 的测试里）：
    三个 locale 的 key 集合相等、代码里引用的 key 都存在、没有内联的用户文案；
  - **占位符数量在 CI 里报数，不阻断**（`make i18n`），数字只能往下走；
  - **硬失败的那条是 `make i18n-strict`**，写进 DoD 的 M2 上线清单。
- 理由：
  - 闸门的意思是「上线前不得有」，不是「下一次提交前不得有」。
  - **一个挡住无关工作的闸门，最后一定会被人关掉**——那时它连上线也挡不住了。
  - 结构性的三条现在就能绿，而且它们防的是另一类错误：
    少一个 key 是某个语言下的一块空白，拼错的 key 是学习者面前的一个异常。
    这两种今天就可能发生，所以今天就该卡。
- 回退成本：低。翻译补齐后，把 `make i18n-strict` 挪进 `make check` 即可。
- 影响范围：`bot/i18n_check.py`、`.github/workflows/ci.yml`、
  `docs/DEFINITION_OF_DONE.md`、D8 之后的所有 PR。

## D-052 message key 按位置识别，不按形状；`_KEY` 后缀是可执行的约定

- 日期：2026-09-19
- 背景：`bot.turn` 是日志事件名（CODING_STANDARDS 第 6 节的 `模块.动作`），
  `attempt.audio_too_large` 是 AppError 的 code（第 5.1 节），
  `bot.welcome` 是文案 key——**三者形状完全一样，只有一种在 `locales/` 里**。
  按前缀匹配的检查会把每一条日志都报成未知 key。
- 选择：只读**构造上就是 message key 的位置**：
  `Reply(...)` 与 `i18n.t(...)` 的第一个参数，以及**名字以 `_KEY` / `_KEYS` 结尾**
  的常量。后者把命名变成一条可执行的约定。
- 理由：
  - 零误报。日志事件与错误码天然落在别的位置上，不需要白名单。
  - 顺带逼出一处真实的命名混淆：`attempts.py` 里的 `TARGET_TEXT_KEY`
    其实是 **JSONB 字段名**，不是文案 key。已改名 `TARGET_TEXT_FIELD`——
    两种「key」长得一模一样，正是这条约定要分开的东西。
- 回退成本：低。
- 影响范围：`bot/i18n_check.py`、`bot/handlers.py`（`_ERROR_MESSAGE_KEYS` 改名）、
  `backend/app/api/v1/attempts.py`（`TARGET_TEXT_FIELD` 改名）。
- 保障：变异 F（把所有调用都当成 key 来源）转红 6 条、
  变异 G（忽略 `_KEY` 常量）与变异 E（不读 `Reply` 的 key）各转红。
- 附带发现：**十五条变异里有五条第一遍存活，其中四条是同一个病根**——
  所有断言都是「仓库现在是干净的」（`findings.unknown_keys == []` 之类），
  而 `== []` 在**检查器彻底瞎掉时同样成立**。
  一个只跑过干净仓库的检查器，没有证明过它能发现任何东西。
  已经给 `run()` 加上 roots / catalogues 入参，专门喂它一棵**故意写坏的目录树**，
  断言它**找得到**。第五条（把 URL 也当成文案）是漏了空串与 None 的用例。
  这是 DoD 1.1 里「断言一个恒真命题」那一类的教科书样本。

## D-053 订单买了什么，存进 `payments.raw_payload` 的命名空间下

- 日期：2026-09-19
- 背景：回调回来时要知道这笔订单买的是哪个档、哪个周期，而 `payments` 表
  **没有这两列**（`DATA_MODEL.sql` 是权威 DDL，D8a 不该为此改表）。
- 选择：下单时把 `{"kruai_order": {"plan": ..., "period": ...}}` 写进
  `raw_payload`；回调回来时把渠道的原始报文并排放在 `{"provider_callback": ...}`，
  **不覆盖**我们自己的那一半。
- 理由：
  - 另外两条路都更糟：把含义编进 order_id（要解析我们自己的 id，且改格式即破坏
    在途订单），或者改 DDL 加两列（D8a 不需要，而改权威 DDL 是更大的动作）。
  - **两半并排放**：对账时两边都要用，而且谁也不是谁的一部分。
  - 命名空间前缀让它们不可能撞车——渠道的字段名不受我们控制。
- 回退成本：低。真要加列时，一条 migration 加上把 raw_payload 里的值搬过去。
- 影响范围：`app/api/v1/payments.py`、D8b、D9。

## D-054 任务额度在**开课**时扣，不在完课时扣

- 日期：2026-09-19
- 背景：D-043 定了「一个任务 = 完成一节课」，那是**计量口径**。
  在哪一步扣减是另一个问题，PRD 未定义。
- 选择：新增 `POST /lessons/{id}/start`，在那里扣一次 `consume_task`；
  额度不足返回 402，并且**回滚**——不留下已开课的痕迹。
  同一节课再次 start 不再扣（`ON CONFLICT DO NOTHING` 判定是否首次）。
- 理由：
  - **付费墙必须是一扇门，不是一张账单。** 在完课时扣减意味着：学习者做完了
    整节课，然后被告知「今天的额度用完了」——工作已经做完，这次拒绝什么也没保护住，
    却把他刚花的时间作废。
  - **重进不重扣**：一节课分三次上完仍然是一节课。按次扣会让这条限额
    变成没人同意过的东西。
  - 顺带填上了 `lesson_progress.status='started'`——DDL 里一直有这个状态，
    而在此之前没有任何代码写过它，PRD 3.3 的流程也是从 `lesson_start` 开始的。
- 回退成本：低。
- 影响范围：`app/api/v1/lessons.py`、`bot/handlers.py`（Bot 开课前先调 start）、D8b。
- 保障：变异 M（开课不扣额度）转红 19 条、变异 N（被拒后仍标记已开课）与
  变异 O（重进再扣一次）各转红。

## D-055 价格是配置，格式为 `币种:最小单位整数`；没定价的币种不许启动

- 日期：2026-09-19
- 背景：PRD 4.3 给了四个 USD 价格，但 CONFIG_REFERENCE 里一个价格项都没有，
  DDL 里也没有商品表。
- 选择：四个配置项（plan × period），每个的值是 `USD:199,KHR:8000` 这样的表。
  启动自检遍历 `SUPPORTED_CURRENCIES` × 可购档位 × 周期，缺一个就拒绝启动。
- 理由：
  - **一个 key 承载所有币种**，而不是 plan × period × currency 个 key：
    加一个币种不该意味着加四个配置项。
  - **金额是最小货币单位的整数**，解析时显式拒绝小数（`USD:1.99` 直接报错并
    指明「float money is a bug」）——在配置文件里写 1.99 的人想的是「元」，
    那正是最可能少收一百倍的地方。
  - **没定价就不许启动**：接受一种收不了钱的币种，症状是某个学习者在结账时
    被我们自己的配置拒绝，而且只在没人测过的那个币种上出现。
    这条在写测试时**当场就抓到了我自己**——我给 KHR 只填了四分之一的价格，
    应用直接起不来。
- 回退成本：低。
- 影响范围：`core/config.py`、`services/entitlements/pricing.py`、
  `services/selfcheck.py`、D8b、D9。
- 保障：变异 K（接受小数价格）、变异 L（没定价按 0 收）、变异 P（自检不查定价）
  各转红。
- 附带发现：变异 I（删掉 `SUPPORTED_CURRENCIES` 校验）第一遍存活——
  因为默认配置下 KHR 既不被接受**也没有定价**，两种拒绝给出同样的 400。
  真正的区别在于「价目表已填但币种还没开」：运维为上线做好了准备，
  不等于已经上线。补了一条那个形态的测试。

## D-056 续费模式在开通那一刻就定死，由渠道能力决定

- 日期：2026-09-19
- 背景：ARCHITECTURE 3.4 要求 auto / manual 双路径，且**业务层不得假设任一种**。
  D8b 才实现那两个定时任务，但订阅行现在就要写对。
- 选择：`renewal_mode = 'auto'` 当且仅当
  `provider.supports_recurring and mandate_ref`；否则 `manual`。
  auto 同时写 `next_charge_at = period_end`，manual 两者都留空。
- 理由：
  - **两个条件都要**：渠道支持代扣但这次没拿到 mandate，也只能走 manual——
    而且 DDL 的 `auto_needs_mandate` 约束本来就不允许另一种组合。
    落在 manual 这一侧是可恢复的（提醒用户再买一次），落在 auto 那侧是收不到钱。
  - **`next_charge_at` 从第一行就写对**：D8b 的定时任务靠它驱动，
    漏写的话第一次续费就是迟到的那一次。
- 回退成本：低。
...
## D-060 续费金额与周期从「开出这个周期的那笔付款」读回

- 日期：2026-09-19
- 背景：续费要知道三件事——档位、周期、币种。档位在 `subscriptions.plan`，
  **另外两个哪都没有**：那张表既没有 currency 也没有 period 列。
- 选择：回读该用户最近一笔成功付款的 `currency` 与 `raw_payload.kruai_order.period`
  （每一笔付款都带着这两样，D-053），据此在价目表里取价。
  续费自己写的付款行同样带上它们，所以下一次续费读到的是上一次续费。
- 理由：
  - **在柬埔寨用错币种是实实在在的事故**：用里尔付款的人被扣 199 美元，
    或者反过来。这不是舍入误差，是两个数量级。
  - 备选是给 `subscriptions` 加两列。那要改权威 DDL 加迁移，而 D8b 不需要它——
    **但那确实是更干净的归宿**：回读付款行的前提是「最近一笔成功付款属于这个订阅」，
    对一个人同时持有两个订阅的情况并不严格成立（升级期内会出现）。
    真要动 DDL 时，这是第一个该加的东西。
  - 读不出来时回落 monthly/USD 并打 error 日志：付了钱的人应该拿到**某样东西**，
    而日志是叫人去看的。
- 回退成本：中。改成列以后要迁移历史订阅（可从付款行推出来）。
- 影响范围：`services/subscriptions/renewal.py`、D9。
- 保障：变异 O（按默认币种扣款）与变异 P（周期一律按月）各转红。

## D-061 重试次数由「本周期内失败的扣款笔数」推出，不加列

- 日期：2026-09-19
- 背景：`SUBSCRIPTION_AUTO_CHARGE_RETRY_DAYS=1,3` 需要知道「这是第几次重试」，
  而 `subscriptions` 没有计数列。
- 选择：数 `payments` 里 `is_recurring_charge=true AND status='failed'
  AND created_at >= period_start` 的行数。
- 理由：
  - **每次扣款本来就该留痕**——成功与失败都写一行，这是动钱的最低要求。
    既然那些行必须存在，再加一个会漂移的计数列就是第二份真相。
  - 续期时 `period_start` 前移，计数自然归零，不需要任何人记得重置它。
  - 成本：每次判定多一条 COUNT。这是个按天跑的任务，不是请求路径。
- 回退成本：低。
- 影响范围：`services/subscriptions/renewal.py`。
- 保障：变异 D（失败的扣款不记录）转红 2 条——不记录就数不出来，
  重试会无限循环。

## D-062 提醒只在**确实送达之后**才记 `reminder_sent_at`

- 日期：2026-09-19
- 背景：manual 路径里，提醒**就是**续费机制——柬埔寨多数渠道不支持代扣，
  不提醒就等于不续。
- 选择：抽一个 `ReminderSender` 协议（`send(...) -> bool`），
  只有返回 True 才写 `reminder_sent_at`；Telegram 实现放 `workers/reminders.py`，
  把 `TelegramError` 当作投递失败而不是崩溃。
  **`TELEGRAM_BOT_TOKEN` 为空时构造即失败**，任务根本跑不起来。
- 理由：
  - **「记为已发但其实没发」是这个任务唯一不能犯的错**：学习者不会收到任何
    预警，只会某天发现档位没了。宁可下一轮再发一次。
  - 协议而不是直接调 Telegram：没有 bot token 也能测——而这正是
    D6 之后 token 一直没到位的现实。
  - 空 token 时**拒绝启动**而不是「跳过投递但照记」，理由同上。
- 回退成本：低。
- 影响范围：`app/workers/subscriptions.py`、`app/workers/reminders.py`。
- 保障：变异 I（未送达也记为已发）与变异 J（去掉防重复）各转红。

## D-063 一次 lapse 扫描里，先判过期再给宽限

- 日期：2026-09-19
- 背景：同一个任务既把「周期结束」推进 grace，又把「grace 结束」推进 expired。
  两件事挨在一起，顺序会决定一个刚刚到期的订阅会不会在同一秒被判死。
- 选择：**先查 expired，再查并写 grace**。
- 理由：反过来的话，一个周期早就结束的订阅会在这一轮先拿到 grace_until，
  而如果那个时间点已经过去（比如周期结束在 30 天前），紧接着的过期判定
  就会把它一起收走——学习者一天宽限都没拿到。
  这类顺序错误不会报错，只会让一部分人少几天权益。
- 回退成本：低。
- 影响范围：`app/workers/subscriptions.py`。
- 保障：变异 N（调换顺序）转红 3 条。

## D-064 auto 订阅只要还排着扣款就不参与 lapse

- 日期：2026-09-19
- 背景：worker 停了一天，一批 auto 订阅的 `period_end` 过去了但还没被扣。
  lapse 任务要不要把它们推进 grace？
- 选择：不。`due_for_lapse` 只收 **manual**，或者
  **`next_charge_at IS NULL`（没有任何东西会来扣它）** 的 auto。
- 理由：因为扣款任务跑晚了就把付费用户降级，比让周期多跑一天糟得多——
  前者是我们的故障转嫁给用户，后者只是我们少收一天。
  而 `next_charge_at` 为空的 auto 订阅确实没有任何东西会管它，那才是真的搁浅。
- 回退成本：低。
- 影响范围：`app/workers/subscriptions.py`、`services/subscriptions/renewal.py`。
- 保障：变异 M（照样 lapse）转红。
- 附带发现：变异 G（`due_for_charge` 去掉 `renewal_mode == "auto"` 条件）
  第一遍**存活**——因为测试里每一个 manual 订阅的 `next_charge_at` 都恰好是空的，
  另一个条件替它挡住了。而 DDL 允许 manual 行带着 `next_charge_at`，
  「按 renewal_mode 判定」才是 ARCHITECTURE 3.4 的规则，
  「靠另一个字段恰好为空」不是。已补一条 manual + 带扣款时间的测试。

## D-065 同一时间只允许一份生效的订阅；升级留到有人决定为止

- 日期：2026-09-19
- 背景：探针发现 basic 用户可以直接买 pro，结果是**两份订阅并存、两份都 auto 续费**——
  一个账号每月扣两笔，而系统里没有任何东西会察觉。
  更糟的是 D-060 的条款回读（取最近一笔成功付款）会让 basic 那份
  按 pro 那笔的 yearly 条款续费。
- 选择：checkout 时若已有 `active` 或 `grace` 的订阅，直接 400 拒绝。
  升级（旧的怎么办、剩余天数怎么算）是**动钱的产品决定**，
  由项目所有者于 2026-09-19 拍板：**现在拒绝，升级以后再说**。
- 理由：
  - 重复扣款是不可接受的一类错误，而且用户往往几个月后才发现。
  - **拒绝不需要任何金额计算**，所以不会引入新的取整与尾差问题（R4）。
  - `grace` 也算生效——它仍然授予权益（ENTITLING_STATUSES），
    在宽限期里买第二份同样会变成两份。
  - `expired` 不拦：那正是 manual 路径要的「再买一次」。
  - 代价：Basic 用户想升 Pro 要等当前周期结束（最多一个月）。
    M2 只有一个 HSK1 课包，Pro 的实时语音要 M3 才上，升级需求现在几乎不存在。
- 回退成本：低。
- 影响范围：`app/api/v1/payments.py`、将来的升级功能。
- 保障：变异 A（去掉拦截）转红 2 条、变异 B（只拦 active）与
  变异 C（连 expired 也拦）各转红。

## D-066 checkout 拿到的 mandate 要存下来

- 日期：2026-09-19
- 背景：`create_checkout` 会返回 `mandate_ref`，而 D8a 把它丢掉了——
  开通时用的是回调里带回来的那一份。回调正常时没有区别；
  **回调丢了走对账兜底时，`mandate_ref=None`，auto 订阅静默变成 manual**。
  探针验证：`AFTER RECONCILE: [('manual', None, False)]`。
- 选择：checkout 时把它写进 `payments.raw_payload` 的命名空间；
  结算时优先用回调里的，没有就用存下来的。
- 理由：用户以为自己开了自动续费，实际在等一封提醒——
  这不是致命错误（manual 是可恢复的那一侧），但它是一次**静默的降级**，
  而那个 mandate 我们本来就已经拿到了。
- 回退成本：低。
- 影响范围：`app/api/v1/payments.py`、`services/subscriptions/settle.py`。
- 保障：变异 H（不存）与变异 I（存了不用）各转红。

## D-067 回调声称的金额要与订单核对

- 日期：2026-09-19
- 背景：探针发现一个**签名正确**的回调声称付了 1 分钱，
  5400 分的 Pro 年费订单照样开通。
- 选择：`settle_order` 收下 `callback.money`，与订单的
  `amount_minor / currency / currency_minor_units` **三项全等**才放行；
  不等就拒绝并打 error 日志。回调**没报金额**时照常放行。
- 理由：
  - **签名证明的是「谁发的」，不是「发的对不对」**。部分支付、金额调整
    在真实收单里都存在，而「一分钱换一年 Pro」不是舍入误差。
  - **三项全等而不只比数字**：199 里尔和 199 分是同一个整数，
    却是完全不同的钱——这正是 `amount_minor` + `currency_minor_units`
    这套约定存在的理由。
  - **不报金额的回调照常放行**：很多渠道的回调本来就不带金额，
    拒绝它们等于为一个行为正常的渠道制造故障。
  - 返回 200 而不是 4xx：金额不会因为重投而改变，让收单机构一直重试没有意义；
    这一笔留在 pending，由对账任务与人去看。
- 回退成本：低。
- 影响范围：`services/subscriptions/settle.py`、`app/api/v1/payments.py`、D9。
- 保障：变异 D（不核对）、E（连没报金额的也拒）、F（只比数字不比币种）、
  G（webhook 不传金额）全部转红。

## D-068 用「每个 provider 都要过」的契约测试给 D9 预留位置

- 日期：2026-09-19
- 背景：`payments/base.py` 用散文写了四条承诺——checkout 幂等、`verify_callback`
  无副作用、不抛异常、如实申报能力。**散文不可执行**，而 ABA adapter（D9）
  要等 M0-3，写它的人未必是写这段抽象的人。
- 选择：新增 `tests/unit/test_payment_provider_contract.py`，
  **按 registry 里的每一个 provider 参数化**。新增一个 adapter 只要在
  `_FACTORIES` 加一行，这些测试立刻管住它，不需要任何人记得。
- 理由：
  - 契约的价值在于**被违反时会响**。今天只有两个 fake，看起来是自测；
    真正的收益在 ABA 落地那天——它的作者会在这里发现问题，
    而不是在某个学习者付了钱却没开通的时候。
  - 最要紧的一条是「验签失败时不得回吐 `order_id` / `money` / `status`」：
    回吐了，调用方就可能在没检查 `signature_valid` 的情况下用它，
    而那正是「凭未验证输入开通订阅」的路径。
  - 同时写了 `docs/aba-integration.md`：把 D9 从一个待办变成一张填空题，
    列清四样缺失的外部事实、改动面（三个文件）、以及
    **`HASH_FIELD_ORDER` 必须用 ABA 文档里的示例签名单独复现**——
    「沙箱能跑通」只证明那一种字段组合是对的。
- 回退成本：无（纯测试与文档）。
- 影响范围：`tests/unit/test_payment_provider_contract.py`、`docs/aba-integration.md`、D9。

## D-069 升级功能留一个有名字的分支，而不是一句注释

- 日期：2026-09-19
- 背景：D-065 把「已有生效订阅时再次下单」定为拒绝，升级留待产品决策。
  问题是：将来做升级时，怎么保证接手的人知道该替换哪里？
- 选择：
  - 拒绝用**独立的错误码** `payment.already_subscribed`（409），
    不与其他支付拒绝混在 `payment.refused` 里；
  - Bot 把它映射到一句自己的文案，学习者得到的是「你已经有订阅了」
    而不是一句泛泛的失败；
  - BACKLOG 新增 **D11**，写明先决条件是产品决策，并写明
    **落地时必须同时把 `subscriptions` 的币种与周期改成真实列**（D-060）——
    因为一人两订阅正是那个回读逻辑失效的场景。
- 理由：
  - 一个有名字的分支是可搜索的；一句 `# TODO: upgrade` 不是。
    验收标准里那句「`payment.already_subscribed` 分支被升级流程取代」
    就是给接手的人的定位符。
  - **不预先写任何折算代码**：按剩余天数折算会引入需要舍入的金额，
    而 R4 禁止浮点、要求显式取整与尾差归属——那些决定只有在
    产品口径定了之后才做得对。现在写等于凭空造一条没人验证过的算钱路径。
- 回退成本：无。
- 影响范围：`core/errors.py`、`api/v1/payments.py`、`bot/handlers.py`、
  `docs/BACKLOG.md`、D11。

## D-070 成本护栏的阈值向上取整，且比较全程走整数

- 日期：2026-09-19
- 背景：PRD 11.3 要求「超过档位上限 1.5 倍时告警并自动限流」。
  遗留约束 L-1 早就把问题挑明了：`COST_ALERT_MULTIPLIER` 是全项目唯一
  接触金额的浮点数，`45 × 1.5 = 67.5`，**67 到底算不算超标需要显式决定**，
  不能让浮点序关系替我们决定。
- 选择：
  - 比率先被**一次性**放大成整数（千分比，1.5 → 1500），
    之后所有比较都是整数运算；放大用 `round` 而不是 `int`；
  - 阈值**向上取整**：67.5 → 68。用 `-(-a // b)`，全程不出现浮点；
  - 判定用 `>=`：花到 68 就限流，花到 67 继续练。
- 理由：
  - 规则的原话是「**超过**上限的 1.5 倍」。67 不超过 67.5，68 才超过。
    向下取整会让一个没到线的人被限流，那是我们自己制造的客诉；
  - 比率只在一处取整，是为了让阈值不依赖于「谁在问」——
    如果每个调用点各自取整，同一个学习者在 `/admin/costs` 上显示未超标、
    在 `/attempts` 上被拦下，两边都没错，但事实只有一个；
  - `round` 而不是 `int`：`1.001 × 1000` 的浮点结果是 `1000.9999999999999`，
    截断得到 1000——运维要的一点点余量会被悄悄抹平，
    变成「乘数写了 1.001，行为却等于 1.0」。单测用这个数钉住。
- 回退成本：低。三个函数都是纯函数，改取整方向只动 `cost_guard.py` 一处，
  阈值边界（67/68）在单测里是显式数字，改了立刻红。
- 影响范围：`services/entitlements/cost_guard.py`、`core/config.py` 注释；
  **遗留约束 L-1 就此消除**。

## D-071 超限返回 429 且不告诉学习者他花了多少钱

- 日期：2026-09-19
- 背景：超过成本上限该回什么？额度用尽是 402（`quota.exhausted`），
  成本超限看起来也像「没额度了」。
- 选择：
  - 独立错误码 `cost.cap_reached`，HTTP **429**，不复用 402；
  - 护栏放在**扣额度之前、调用打分器之前**——被拦下的那次练习
    不扣次数、不产生 `cost_ledger` 行、不写 attempt 行；
  - 告警写在服务端日志（`cost.cap_exceeded`，error 级，带花销/阈值/上限），
    面向学习者的文案里**不出现任何金额**。
- 理由：
  - 402 的含义是「你买的份额用完了，可以再买」，而成本超限
    **不是学习者能花钱解决的**——他已经付过钱了。让他去续费是误导。
    429「你太快了，缓一缓」才是实话；
  - 顺序是这条护栏的全部意义：放在扣额度之后，被限流的人还要赔上一次次数，
    等于因为我们的成本问题罚他。变异测试里「护栏后移一行」必须被测到；
  - 成本是我们的内部数字。告诉学习者「你这个月花了我们 0.68 美元」
    既无用又难堪，还等于把单位成本公开给任何注册用户
    （CLAUDE.md 第 6 节：对外可见的表述要谨慎）。
- 回退成本：低。错误码是新增的，没有调用方依赖旧行为。
- 影响范围：`core/errors.py`、`api/v1/attempts.py`、`bot/handlers.py`、三份 locales。

## D-072 成本月份按 UTC 自然月，不跟学习者的时区

- 日期：2026-09-19
- 背景：「每用户每月成本」的「月」是谁的月？C5 已经把**每日额度**
  定成按学习者本地日（PRD 的口径是「学习者的一天」），
  成本上限看起来应该照抄。
- 选择：按 **UTC 自然月**，从当月 1 日 00:00 UTC 起算。
- 理由：
  - 两个「周期」问的不是同一件事。日额度是产品承诺，属于学习者，
    所以按他的日；成本上限是**我们自己账单的口径**，
    一张账单覆盖若干时区的人，按人各算各的月，加起来对不上任何一张账单；
  - 按 UTC 还让 `/admin/costs` 与护栏用同一个窗口，
    看板上的数字和拦截的依据是同一个。
- 回退成本：低。`month_start()` 是单独一个函数，换口径只动它，
  单测已经把「上个月的花销不算进这个月」钉住。
- 影响范围：`services/entitlements/cost_guard.py`。

## D-073 一个请求只持有一条数据库连接

- 日期：2026-09-19
- 背景：G-D 验收在跑「额度扣减在并发下正确」这一条时，用 15 个并发的
  `POST /attempts` 一探，结果不是超扣，而是**整批挂死 30 秒后 500**。
  原因不在额度逻辑，在连接：
  - 请求本身持有一条连接（`SessionDep` 从第一次查询一直持到 commit）；
  - 而扣额度（`Entitlements`）、日额度翻篇（`QuotaReset`）、
    记账（`CostLedger`）**各自用 session_factory 另开一条**——
    这三处开独立事务是对的（记账必须在请求事务回滚后依然留下），
    问题是它们开的时候，请求那条还握在手里。
  - 于是「同时在飞的请求数 = 池容量」时，每个人都握着一条、
    每个人都在等一条只有别人能还的连接。谁也动不了，直到 `pool_timeout`。
  - 实测：池容量 15 时，14 并发 0.7 秒全绿，15 并发 30.4 秒、5 个 500。
    悬崖之下完全看不出任何征兆。
- 选择：
  - **把所有读挪到前面，读完 `await session.commit()` 把连接还回去**，
    再做那些要自己开 session 的事（扣额度 / 打分 / 记账），
    最后写 attempt 与 mastery 时重新取一条。一个请求任一时刻只握一条；
  - `_reset_quota_if_due` 不再接 `session`：它原先在里面读时区，
    那是最后一处「握着还要再开一条」的地方。时区改由调用方读好传进去；
  - 连接池三个参数 `DB_POOL_SIZE` / `DB_MAX_OVERFLOW` /
    `DB_POOL_TIMEOUT_SECONDS` **收进配置**，不再吃 SQLAlchemy 的默认值
    （5 + 10）。超时默认 10 秒而不是 30：与其让语音上传挂半分钟，
    不如早点告诉学习者重试。
- 理由：
  - 顺带修掉一件更贵的事：打分是一次**跨网络的外部调用**，
    原先整个调用期间请求都攥着一条连接不放。真实 Azure 一次一两秒，
    等于每条连接的吞吐被外部延迟锁死。现在那段时间一条都不占；
  - 不把三处独立事务并回请求事务——那会让「请求回滚后记账还在」失效，
    而那正是 CLAUDE.md 第 8 节要的；
  - 回归测试取 `DB_POOL_SIZE + DB_MAX_OVERFLOW + 1` 并发，
    数字从配置推出来而不是写死：池调大了，测试跟着调大，悬崖始终被测在外侧。
- 同一个悬崖在另外两个端点上也实测到了，一并修掉：
  - `GET /me/entitlements`：读完 plan 与时区就还连接，再做 reset 与 snapshot；
  - `POST /lessons/{id}/start`：把 reset 提到写 `lesson_progress` 之前，
    并给 `consume_task` 加一个可选的 `session=` —— 扣额度改在**请求自己的事务里**跑。
    那条语句仍是同一条带守卫的 UPDATE，防竞争不依赖于谁来执行；
    换来的是「标记开课」与「扣掉这次任务」同生共死，
    原先它们分属两个事务，中间断电就会开了课却没扣额度。
- 回归测试用一个**故意很小的池**（2 条、不许溢出、等 2 秒），
  而不是对着线上的 30 条去凑 31 个并发：
  池小的时候，任何「一次要两条」的端点每次都卡住；池大的时候只在时机对上时才卡，
  而一个时灵时不灵的测试比没有更糟。每个请求还必须来自**不同的学习者**——
  同一个人重复打同一个幂等端点，只有第一次会走到要第二条连接的那段代码。
- 回退成本：低。三处改动各自独立，`session=` 是可选参数，不传就是原行为。
- 影响范围：`api/v1/attempts.py`、`api/v1/me.py`、`api/v1/lessons.py`、
  `services/entitlements/quota.py`、`core/db.py`、`core/config.py`、
  `.env.example`、`docs/CONFIG_REFERENCE.md`。
  **新端点照这个规矩来**：请求 session 上的读写与 session_factory 开的事务
  不要在时间上重叠，新端点加进 `test_pool_pressure.py` 的清单里就有人替你盯着。

## D-074 Free 的日额度由成本上限倒推，不是拍的

- 日期：2026-09-19
- 背景：G-D 验收第 10 条要求「单用户月成本估算未超 `COST_CAP_*`」。
  第一次真把这个数算出来，就发现**额度与上限是分别定的，对不上**：
  - PRD 11.1 给 Basic 的假设是每月 20 分钟录音，11.2 给它 $0.45 的上限
    → 一次 4 秒的 drill 约 **0.15 美分**；
  - `LIMIT_FREE_DAILY_ATTEMPTS=10` → 每月 300 次 → 20 分钟 → 约 **$0.45**；
  - 而 Free 的上限是 **$0.15**。差 3 倍。
  - 实际后果不是超支，是**限流**：D10 的护栏在 $0.23 触发，
    满额使用的免费用户每个月第 15 天左右被 429 挡住，直到下个月。
    也就是说「例外」会变成每月必然发生的常态。
- 选择：**`LIMIT_FREE_DAILY_ATTEMPTS` 从 10 改为 3**
  （项目所有者 2026-09-19 决定）。
  每月 90 次 ≈ 13.5 美分，落在 $0.15 以内，且永远够不到 $0.23 的护栏。
- 理由：
  - 两个数不是独立的。日额度是给学习者的承诺，成本上限是我们的预算，
    前者买的东西超过后者付得起的时候，差额只能由护栏来堵——
    而护栏是设计给异常用的，不是给每个月的第 15 天用的；
  - 宁可每天被稳定地告知「今天到此为止」，也不要月中被突然切断。
    前者是一份计划，后者是一次故障；
  - 不动 `COST_CAP_FREE_USD_CENTS_MONTHLY`：免费档没有收入抵消，
    把上限提到 Basic 的水平等于 3000 个活跃免费用户每月烧掉 $1350。
- 单测把这条关系钉住了（`test_cost_guard.py`）：
  按 PRD 11.1 自己的单价算出来的免费月成本必须 ≤ 免费上限，
  且满额免费用户不得触发护栏。**以后谁调高额度而不调高上限，测试就红。**
- 回退成本：一个配置值。
- 影响范围：`core/config.py`、`.env.example`、`docs/CONFIG_REFERENCE.md`、
  `tests/unit/test_cost_guard.py`，以及若干断言 10 的旧测试。

## D-075 支付通道的调用要记账，但成本记 0

- 日期：2026-09-19
- 背景：CLAUDE.md 第 8 节说「每一次外部 API 调用都必须写入 `cost_ledger`」，
  G-D 审计发现**支付通道的三处调用一处都没记**：
  `create_checkout`（下单）、`query_status`（对账查单）、
  `charge_recurring`（续费代扣）。
  难点在于：ABA 的手续费按交易额抽成，如果照实记进 `cost_ledger`，
  就会被 D10 的护栏当成**该学习者的成本**——
  等于「付钱给我们」这个动作会让他离被限流更近一步。
- 选择（项目所有者 2026-09-19 决定）：
  - 三处调用都用 `external_call` 包起来，`ref='payment'`、`unit='calls'`、
    `cost_usd_cents_est=0`；
  - 带上 `user_id`：金额是 0，但「这次调用是关于谁的」正是客服会问的；
  - 手续费本身不进这张表，将来要核算就另走一个不按用户聚合的口径。
- 理由：
  - 第 8 节那句话的重点其实有两层，只有一层是钱。另一层是
    **「调用发生过就得留下痕迹」**——调用次数、失败率、哪个通道在抽风，
    这些在 `/admin/costs` 上看得见，比只躺在日志里有用得多。
    记 0 保住了第二层，而不牺牲第一层的正确性；
  - 把手续费按用户记进去，会让护栏反着工作：越是付费的学习者越接近被限流。
    这不是舍入误差，是规则本身装反了。
- 回退成本：低。`ref` 是自由文本（DATA_MODEL.sql 无 CHECK），
  改成本口径只动三处 `entry.record(...)`。
- 影响范围：`api/v1/payments.py`、`workers/payments.py`、
...
- 回退成本：低。
- **2026-09-20 已核实**：六个值与级别归属对照两个独立来源一一吻合
  （题量也对得上：初级 听后重复 15 / 听后回答 10 / 回答问题 2；
  中级 听后重复 10 / 看图说话 2 / 回答问题 2；
  高级 听后复述 3 / 朗读 1 / 回答问题 2）。
  顺带澄清一处容易抄错的地方：**高级第一部分是「听后复述」，不是「听后重复」**——
  有资料把两者混写，本项目用的是前者，与大纲一致。
- 影响范围：`pipeline/seed_schema.py`、`pipeline/seed/README.md`。

## D-081 高棉语解释由 LLM 起草、母语者校对，不由母语者从零写

- 日期：2026-09-19
- 背景：D-079 让 `km_explanation` 可以留 `[[km:...]]` 占位，但没说这些占位符
  最后由谁填。两条路的工作量差一个数量级，且涉及「产品对外说的话有多少是机器写的」，
  属于 CLAUDE.md 第 6 节「对外可见」，不能由实施方自行决定。
- 选择：**项目所有者于 2026-09-19 拍板走 LLM 起草 + 母语者校对。**
  依据是 PRD 6.1 原本就把「高棉语解释」列为 `generate.py` 的产出之一，
  这条路是 PRD 的默认设想，不是新增范围。
- 因此：E2 录 HSK1 骨架时 `km_explanation` 一律写占位符，不等母语者；
  E3 的 `generate.py` 为带占位符的 concept 生成高棉语解释草稿；
  E5 的抽检是这批文本唯一的质量下限。
- **两个随之而来的约束**：
  1. **E5 的 10% 抽样必须把高棉语解释单独分层抽**，不能和目标句混在一起按总量抽。
     否则 90% 合格率可以靠「抽到的大多是中文句子」达成，而机器写的高棉语正是
     最需要被看见的那部分。
  2. **真实 LLM 跑一遍 HSK1 是花钱的**（CLAUDE.md 第 6 节第一类）。`LLM_PROVIDER=fake`
     时不花钱，E3 全程可以用 Fake 验收；切到真实 provider 批量生成前要先算账，
     并按 PRD 11.3 记入 `cost_ledger` 的 `ref='content_production'`，不混进运营成本。
- 备选与放弃原因：母语者从零写质量最高，但 M1 会被人力进度直接卡住，
  且在找到人之前这段工作无法开始。质量下限由 E5 抽检兜底。
- 回退成本：中。若抽检发现机器写的高棉语不可用，退路是把 E5 的抽样比例拉到 100%
  并转为「母语者改写」，seed 与 pack 结构都不用动——但那时已经付过一次生成成本。
- 影响范围：E2、E3、E5；`pipeline/seed_schema.py` 的占位符规则不变。

## D-082 HSK1 的 concept 与大纲语法点一一对应，并把大纲编号存进 `standard_ref`

- 日期：2026-09-19
- 背景：E2 的验收是「HSK1 全部 concept 有 seed」。「全部」需要一个可检验的定义，
  否则只能靠人在 M1 时对着一份 260 页的扫描件数。
- 依据：《国际中文教育中文水平等级标准》(GF 0025-2021) 附录 A 把一级语法点编号为
  **一01 至 一48**，第 49 条开始就是二级。**这是一个封闭列表**，所以完整性可以写成测试。
- 选择：
  1. 一条语法点 = 一个 concept，**不做归并**。`seed_schema.py` 加一个可选字段
     `standard_ref`（如 `一01`），文件内唯一；`tests/test_seed_hsk1.py` 断言
     hsk1.yaml 恰好覆盖 一01–一48，多一条少一条都红。
  2. 例句取自大纲每条下面的例句。国家标准属 PRD 5.5 白名单，来源在 `sources[].detail`
     里写明「含其例句」。
- 备选与放弃原因：按教学直觉归并（比如把 一21 动态助词「了¹」和 一41 完成态合成一个）
  文件会更好看，但完整性从此无法证明。**哪些 concept 进哪一课是后面的事**
  （`lessons.concept_ids`），不该在 seed 阶段提前把选择权用掉。
  代价是 48 条里有几条是句子成分描述（一24「名词可以作主语」）而不是学习者会去练的句型，
  以及少量重叠——这是可见的、可解释的冗余，比不可证的完整性好。
- 回退成本：低。归并只需改 seed 文件与那条测试的期望集合。
- 影响范围：`pipeline/seed_schema.py`、`pipeline/seed/zh-hsk3.0/hsk1.yaml`、
  `pipeline/tests/test_seed_hsk1.py`。

## D-083 HSKK 任务类型按一条明说的规则分三层，不逐条拍脑袋

- 日期：2026-09-19
- 背景：D-080 定下了六个合法取值，但没说某一条语法点该配哪几个。48 条逐条凭感觉配，
  结果无法复核，也无法在改动时发现漂移。
- 选择：一条规则，写在 hsk1.yaml 的头部注释里，并写成测试：
  - **全部 48 条** → `listen_and_repeat`：一级语法点都可能出现在「听后重复」的句子里；
  - **问句与提问方法**（一04、一33、一45–一48）→ 再加 `listen_and_answer` +
    `answer_questions`：这六条直接决定学习者听到的问题长什么样、以及怎么答；
  - **能独立成句、可直接作为口头回答的句型** → 再加 `listen_and_answer`。
- `test_seed_hsk1.py` 把第二层写成数据断言（`answer_questions` 的集合必须恰好是那六条），
  所以后来人改了配比会在 CI 里看见，而不是在内容里看见。
- **待确认**：这是按 HSKK 初级三个部分的形式做的粗分层，不是考试院的官方映射。
  和 D-080 一起，在 M1 内容 review 时请教学顾问确认一次。
- 回退成本：低，改 seed 与那条断言。
...
## D-088 去重在同类句子之间做，不跨类

- 日期：2026-09-19
- 背景：PRD 6.2 第 7 条只说"句子级 MinHash 去重，相似度 > 0.9 拒绝"，没说哪些句子之间比。
  一个 concept 的对话示范答案，和它的目标句长得一样，是很自然的事——学习者刚练完那句话。
  按全草稿一锅比，这种情况每个 concept 都会被判重复。
- 选择：分三组各自内部比——目标句 / 对话提示 / 示范答案——**不跨组比**。
- 理由：两条只差一个标点的目标句，是浪费掉一个练习位；示范答案复用刚练过的句子，是课在起作用。
  这两件事长得一样，但不是一回事。
- 回退成本：低，合并成一个列表即可。
- 影响范围：`pipeline/validate.py` 的 `validate()` 与 `check_duplicates`。

## D-089 词表是数据不是代码，缺失即硬失败；分词用词表自己做

- 日期：2026-09-19
- 背景：规则 1「词表越界」需要该等级的词表。两个问题：词表放哪，以及中文没有空格，
  怎么判断"这句话里的每个词是否在表内"。
- 选择：
  1. 词表是 `pipeline/wordlists/<language>/<level>.txt`，一行一词，`# source:` 记来源（R7）。
     **缺文件时 `validate.py` 退出码 2，而不是报告"零违规"**——一条因为数据缺失而通过的规则，
     比没有这条规则更糟：构建变绿，等级边界实际上没人把守。
  2. 分词用**词表自身做最长匹配**：从左往右取"在表内的最长片段"，覆盖不掉的就是越界。
     不引入 jieba 之类的分词器。
- 理由：分词器自带一套词汇，和我们的词表恰恰会在最关键的地方产生分歧
  （它认识"咖啡"所以切得出来，但"咖啡"不在 HSK1 表里——我们要的正是这个否定回答）。
  用词表自己分词，问题和答案是同一件事。
- 已知取舍：贪心匹配，不做全局最优切分。极少数情况下"先取短词才切得通"的句子会被误报为越界。
  代价是一个人来读的假阳性，不是一个静默通过的越界词。
- **未落地**：`zh/hsk1.txt` 还不存在，所以规则 1 现在跑不了真实数据（`make validate` 退出 2）。
  GF 0025-2021 的词汇表在 R7 白名单内，且一级词表**带 1–500 的连续编号**，
  这让抽取本身是可校验的（编号不能断、必须止于 500），再加上词表自带拼音列，
  可以和 `pypinyin` 交叉验证。这是一件独立的数据录入工作，不在 E4 的代码范围内。
- 影响范围：`pipeline/wordlist.py`、`pipeline/validate.py`、`pipeline/wordlists/`。

## D-090 拼音比对接受多音字的全部读音

- 日期：2026-09-19
- 背景：PRD 6.2 第 3 条要求用 `pypinyin` 生成拼音与模型输出比对，不一致则拒绝。
  但"一致"按哪种口径？`了` 读 le 也读 liǎo，`不` 在四声前变调，`的/地/得` 各有多读。
  按单一读音比，每一条用到这些字的句子都会误报——而 HSK1 几乎每句都有。
- 选择：用 `heteronym=True` 取该字的**全部**读音，模型写的音节只要命中其中之一就算通过。
- 理由：这条规则真正要抓的是"模型写的拼音不属于它写的那些字"，而不是"模型没选对上下文读音"。
  后者需要上下文消歧，`pypinyin` 也给不出可靠答案；硬做只会让规则因为噪声太大而被关掉。
  声调仍然是比对的一部分——同音不同调照样拒绝（有单测）。
- 回退成本：低。将来若引入带上下文的注音器，收紧成单一读音即可。
- 影响范围：`pipeline/validate.py` 的 `check_pinyin`。

## D-091 HSK1 词表录入：转录表是正本，词表由它派生，两道机械检查守住

- 日期：2026-09-20（解除遗留约束 L-11）
- 背景：规则 1「词表越界」需要一级词表。原件是 260 页无文字层的扫描件，
  只能靠人眼转录——而"转录对不对"这个问题必须有机械答案，不能靠"我看过了"。
- 选择：
  1. **`hsk1.source.tsv` 是正本**：编号 + 词（按表中写法）+ 拼音（按表中写法），
     附 `# source:` 行。`hsk1.txt` 由它派生（`make wordlist`），测试断言两者一致——
     所以没人转录过的词不可能出现在词表里，手改词表会红。
  2. **两道检查**：编号必须 1–500 连续不断（抓掉行、重行）；
     每行的汉字必须与它自己那列拼音在 `pypinyin` 下对得上（抓看错字）。
     第二道是关键：两列是分别读出来的，看错一个字，它几乎不可能仍与旁边的拼音相符。
- 结果：500 行编号连续；**没有一行在汉字上对不上**；47 行只差轻声/变调写法
  （表里写 爸爸 bàba、不大 bú dà，词典形是 bàbà、bù dà），这一类被明确允许——
  它只差声调符号，不差音节，而看错字会改变音节。
- **这不等于转录一定正确**，只是把"可能错在哪"收窄并命名了：
  仍可能存在"字错了但拼音恰好也能对上"的情况（例如同音同调的形近字）。
  母语者复核时优先看这一类。
- 表中记法的展开规则（`expand_entry`，有单测）：`爸爸|爸` 是两个词；
  `白（形）` 括号是词性；`第（第二）`、`子（桌子）` 括号是举例不是词形；
  `有（一）些` 括号在词中间，两种写法都是词；`好玩ㄦ` 出 好玩 与 好玩儿。
  判据是**括号在词尾还是词中**，不是括号里写了什么。
- **顺带发现两件事**：
  1. 分词漏了弯引号（U+201C/201D）不算标点，导致标准自己的例句
     `他高兴地说：“我明天回家。”` 被判越界。已修——这是拿真实数据跑才会暴露的 bug。
  2. **标准的语法附录例句会用它自己词汇表里没有的词**：一13 用了 上海（表里只有 北京），
     一37 用了量词 张（在 站 和 找 之间缺位）。两条都已核对，不是转录错误。
     这意味着规则 1 会拒绝标准自己印出来的句子——E3 生成时把例句当"级别与形式的参照"
     而不是可直接上架的内容，这个决定因此更站得住。已写成测试里的显式白名单。
- 回退成本：低。改一行是改 `source.tsv` 再 `make wordlist`。
- 影响范围：`pipeline/wordlists/zh/`、`pipeline/wordlist.py`、`pipeline/build_wordlist.py`。

## D-092 第三方数据的第二轮交叉验证（2026-09-20）

- 日期：2026-09-20
- 背景：仓库里有四类来自第三方的真实数据，各自的验证强度并不一样。
  在 E5 之前把它们一次性再核一遍，结果记录如下。
- **HSK1 词表（500 条）**：新增两道独立检查。
  1. 查重口径改成「词 + 读音」——原先按词查，把 地(de/dì)、干(gān/gàn)、还(hái/huán)
     三行误报成重复。它们是大纲有意列两次的多音字条目，现在写成断言。
  2. **词典序检查**：表按音节字母序排，同音节再按声调 1<2<3<4<轻声，同音不同字则按字头分组。
     500 行全部符合，**倒退只出现在 16 处换字头的位置**（地→弟→第、飞→非、他→她、坐下→做……），
     没有一处无法解释。这是继编号连续、汉字↔拼音互证之后的第三道独立验证。
  3. 抽查第一页与最后一页共 90 行（18%）重读原件：词与拼音全部一致；
     发现 7 行记法失真——`出来 chū∥·lái` 这类离合词标记被我写成单斜杠，已改。
     不影响任何派生数据（两种记法都会被剥掉），但转录正本应当忠实。
- **48 条语法点**：逐页重读附录 A，编号、标题、96 条例句**全部一致，无一处抄错**。
  但又发现 4 条 `pattern` 是带省略号的摘要而非大纲的封闭清单——
  一01（方位词 12+12 只写了 8 个）、一07（缺 二/两 的区分与 十、百）、
  一43（缺「元」）、一44（「差」的位置写反了）。已补全。
  这与此前 一04、一06 是同一类问题：**大纲给清单时，pattern 必须照抄完整**，
  因为 E3 正是按 pattern 生成句子，漏掉的用法在整级内容里都不会出现。
- **HSKK 六种任务类型**：见 D-080 的核实记录。
- **`seed_sources.py` 的许可声明**：逐条对照来源本身，
  **查出一处错误：CC-CEDICT 是 CC-BY-SA 3.0，不是我写的 4.0**（PRD 只写了 "CC-BY-SA"，
  版本号是我自己补的）。已改。kh.moeys.oer（CC-BY 3.0）与 corpus.tatoeba（CC BY 2.0 FR）
  经原站确认无误。英文侧几条（Oxford、GSE、CEFR、IELTS）**未经核实**，
  注册表里已写明它们记的是「状态」不是「许可」，真正用之前必须查。
- **仍然拦不住的一类**：形近且同音同调的字（做/作、象/像）。
  三道机械检查都看不出，只能靠第二双眼睛。抽查的 90 行里未发现此类问题，
  但这不能推广到全部 500 行。母语者复核时请优先看这一类。
- 影响范围：`pipeline/wordlists/zh/hsk1.source.tsv`、`pipeline/wordlist.py`、
  `pipeline/seed/zh-hsk3.0/hsk1.yaml`、`pipeline/seed_sources.py`、
  `pipeline/tests/test_wordlist_hsk1.py`。

## D-093 抽样按内容种类分层、可复现，CSV 带 BOM，判定分三档

- 日期：2026-09-20
- 背景：E5 只被要求「10% 抽样导 CSV」。但 M1 的验收是「母语者抽检合格率 ≥ 90%」，
  也就是说这个 CSV 产生的那个数字要能作数，于是抽样怎么抽本身是决策。
- 选择：
  1. **按种类分层抽**（落实 D-081）：高棉语解释 / 目标句 / 替换项 / 对话，各自按比例抽。
     一个 concept 有 1 段解释、8 个目标句、12 个替换项、3 个对话；
     一锅抽 10%，母语者拿到的大半是替换项——十来个词，
     而 90% 合格率就可以在**没人读过一段机器写的高棉语散文**的情况下达成。
     那恰恰是最容易出问题的部分。
  2. **每层至少 1 行**：3 个对话的 10% 是 0，而没人看过的那一层等于没有证据。
     取整用 ceil，因为验收写的是比例下限不是目标值。
  3. **可复现**：不给 `--seed` 时，随机种子只由 draft 的语言、等级和比例决定，
     所以两个人导出同一份 draft 会拿到同一批行，一条判定也就能追回它对应的那一行。
     种子**不能用 `hash()`**——Python 的字符串哈希按进程随机化，
     那会让同一份 draft 每次抽出不同的行（与 minhash.py 里避开的是同一个坑）。
     有一条测试用两个不同的 `PYTHONHASHSEED` 跑子进程来验这件事。
  4. **CSV 用 utf-8-sig**：Windows 版 Excel 读没有 BOM 的 UTF-8 会按本地代码页解码，
     高棉文和中文全变乱码——而撞上这件事的人是译者，他会报告「文件坏了」，
     不会报告「编码错了」。三个字节买一个不会发生的事故。
  5. **判定三档**：`ok` / `awkward` / `wrong`。「看得懂但没人会这么说」正是请母语者来判断的东西，
     跟「错」不是一回事；只给两档会把它挤进其中一边，而两边都不对。
- 说明文字**打印在终端而不是写进 CSV**：带前言的 CSV 没有一个表格软件能正确打开。
- 回退成本：低。
- 影响范围：`pipeline/review_export.py`、`Makefile`、`.gitignore`。

## D-094 未过校验的草稿默认不打包；强行打包会在 pack 里留痕

- 日期：2026-09-20
- 背景：PRD 6.1 的顺序是 validate → 抽检 → build，但没说 build 遇到未校验的草稿该怎么办。
  而 E6 的验收要求「FakeTTS 下产出完整 pack」——FakeLLM 的占位内容必然过不了校验，
  所以「一律拒绝」会让这条验收无法演示。
- 选择：**默认拒绝**，`--allow-invalid` 可强行打包，但 pack 里会记下 `validated: false`，
  E7 `import_pack.py` 据此拒绝入库（除非再次显式强制）。
- 理由：命令行上的一个开关只活在打字的那个 shell 里；写进 pack 的那个布尔值会跟着文件走到入库那一刻。
  冒烟跑得通，真内容进不了库，两件事都成立。
- 另一种情况：**该等级没有词表时也拒绝**（退出码 2）。"无法校验"不等于"通过校验"，
  不能因为规则跑不了就把没人检查过的内容固化下来。
- 回退成本：低。
- 影响范围：`pipeline/build_pack.py`、`pipeline/pack.py` 的 `validated`、未来的 E7。

## D-095 音频按内容寻址：文件名就是 text+voice+rate 的哈希

- 日期：2026-09-20
- 背景：`media_assets.source_text_hash` 在 DATA_MODEL 里的用途是「文本变更检测，变更则媒体过期」，
  但没规定怎么算。
- 选择：哈希覆盖**文本、逻辑音色、语速**三者，并且**哈希值本身就是文件名**。
  于是同一句话出现在两个 concept 里只合成一次、只存一份；
  改一个字、换一个音色、或把语速从 0.9 改成 1.0，都会得到不同的 id，旧音频自然过期。
- 理由：只哈希文本的话，把中文示范的语速从 0.9 调成 1.0 不会改变任何哈希，
  于是整级的音频仍然是慢速的，而 pack 看起来完全正确——这是只有人去听才会发现的一类错误。
- 附带效果：HSK1 的 fake 草稿里 1296 段文本去重成 27 个文件。真实内容重复率低得多，
  但替换项这一层本来就大量复用，真金白银省在那里。
- pack 另有一个 `checksum`（sha256 覆盖除它自身以外的全部内容），对应 `content_packs.checksum`，
  入库时用来发现被改过的 pack。
- 回退成本：低，但改哈希口径等于让现存音频全部过期、需要重新合成一次。
- 影响范围：`pipeline/pack.py`、`pipeline/build_pack.py`。

## D-096 `--with-video` 开关存在，但执行即拒绝

- 日期：2026-09-20
- 背景：BACKLOG E6 要求有这个开关，且注明「默认关，S2 前不实现视频分支」；
  PRD 第 14 节把「M1–M2 阶段生成任何视频资产」列为非目标。
- 选择：参数与 `PIPELINE_WITH_VIDEO` 配置都保留，但任一为真时**直接退出码 2**，
  错误信息指向 PRD 14 与 7.2 的进入条件。pack 里保留 `with_video` 字段（恒为 false），
  这样将来真有带视频的 pack 时是可区分的。
- 理由：留一个「什么都不做」的开关，比留一个半实现的视频分支安全——
  后者会在内容还在改的时候被人顺手打开，而视频每分钟 $1–4 的重渲染成本正是 PRD 要避免的。
  R6 说「非目标不得实现，包括顺手加一下」，这就是那条规则的具体形状。

...
| L-10 | **PRD 11.3 的成本护栏（「超过档位上限 1.5 倍时告警并自动限流」）没有任何 BACKLOG 条目认领。** D5 把 `/admin/costs` 做出来了，但它只呈现花销，不比较上限、不告警、不限流。而 PRD 第 11 节把单位成本上限写成**硬约束**，M2 验收清单里也有「单用户月成本估算未超 `COST_CAP_BASIC_USD_CENTS_MONTHLY`」这一条——目前这条只能靠人去看看板。实现时必须同时处理 L-1（浮点比较要显式取整）。 | M2 验收之前；或第一次有真实付费用户产生持续成本时 | `app/api/v1/admin.py` 的模块 docstring；`core/config.py` 的 `cost_alert_multiplier` |

...
### 文案
- [ ] 用户可见字符串零内联（`make check` 里的结构性检查已覆盖）
- [ ] 新增 key 三个 locale 都加了（缺的用 `[[km:key]]` 占位，不得回落英文）
...
**这不是形式主义。到目前为止有七次是变异测试而不是通过的测试发现了问题：**
...
| D3 判断评测失败的两个条件里，只有一个被测到（Fake 总是同时满足两者） | 真实 provider 在 `ok=False` 时带回部分分数 → 照样扣额度、写库、推 mastery |
| D3 额度返还的「不得为负」下限完全没被覆盖 | 重置恰好插在扣减与返还之间 → 计数器 -1，白送额度，列上无 CHECK 不会报错 |
| D7 的 i18n 检查只跑过干净仓库，四条断言都是 `== []` | 检查器整个瞎掉，测试照样全绿——「没发现问题」和「发现不了问题」长得一样 |
...
2. **变异要改语义，不要改语法。** 改出 `SyntaxError` 的「变异」必然转红，
   什么也没证明。
3. **坏掉的 runner 会把每一条变异都报成「抓住了」。** 判据是退出码非零，
   而「测试失败」与「pytest 根本没跑起来」的退出码完全一样。
   D1 第一轮十二条全红，实为子进程继承了 macOS 的 `__PYVENV_LAUNCHER__`，
   venv 里的 pytest 直接 `ModuleNotFoundError`。**要求输出里出现 pytest 的
   统计行**（`N failed, M passed`），拿不到就当作这一轮无效，而不是成功。
...
- [ ] **`make i18n-strict` 通过**：`locales/km.json` 与 `zh.json` 不得有 `[[km:...]]` 占位符。
      这是**上线闸门**，不是 PR 闸门——结构性检查（三个 locale key 一致、无未知 key、
      无内联文案）在每个 PR 的 `make check` 里就会失败，占位符数量只在 CI 里报数
      （见 `bot/i18n_check.py`）

...
按 `docs/BACKLOG.md` 的顺序推进。每个任务标注了依赖、状态与是否被 M0 阻塞。**不要跳过顺序挑"有意思的"做。**

**做完一个条目就更新它的状态列**（填上合并的 PR 号）。那一列是下一个接手的人判断"现在做到哪了"的唯一依据——git 历史要翻，DECISIONS 要读，状态列一眼就够。一张说谎的进度表比没有进度表更费时间。

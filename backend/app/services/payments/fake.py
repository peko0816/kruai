...
#: Markers that steer the fake's answers, so failure branches are reachable
#: without a broken acquirer. Same idea as FakeScorer's __BAD__.
#:
#: FAILED_MARKER is honoured in the mandate as well as the order id, because a
#: recurring charge's order id is generated by the renewal job rather than by
#: the caller — the mandate is the only part of that call a test controls.
...
class FakePaymentProvider(PaymentProvider):
    ...
    async def charge_recurring(
        ...
        if FAILED_MARKER in order_id or FAILED_MARKER in mandate_ref:
            return ChargeResult(
                ok=False,
                status=PaymentStatus.FAILED,
